                ...
                <div class="overflow-x-auto">
                    <table class="w-full divide-y divide-[#a4b0af]/20">
                        <thead class="bg-[#eff2f2]">
                            <tr>
                                <th scope="col" class="group px-4 py-3 text-left">
                                    <div class="flex items-center gap-1 cursor-pointer" wire:click="order('id')">
                                        <span class="text-xs font-medium text-[#5d5c64] uppercase tracking-wider hover:text-[#28b0a3] transition-colors duration-300">N°</span>
                                        <i class="fas fa-sort text-[#777779] group-hover:text-[#28b0a3]"></i>
                                    </div>
                                </th>
                                <th scope="col" class="group px-4 py-3 text-left">
                                    <div class="flex items-center gap-1 cursor-pointer" wire:click="order('tiposancion')">
                                        <span class="text-xs font-medium text-[#5d5c64] uppercase tracking-wider hover:text-[#28b0a3] transition-colors duration-300">Tipo</span>
                                        <i class="fas fa-sort text-[#777779] group-hover:text-[#28b0a3]"></i>
                                    </div>
                                </th>
                                <th scope="col" class="group px-4 py-3 text-left">
                                    <div class="flex items-center gap-1 cursor-pointer" wire:click="order('descripcion')">
                        ...
                        <tbody class="divide-y divide-[#a4b0af]/20">
                            @foreach ($sancions as $sancion)
                                <tr class="hover:bg-[#eff2f2]/50 transition-colors duration-200">
                                    <td class="px-4 py-3">
                                        <div class="text-sm text-[#1e1e1f] font-medium">{{$sancion->id}}</div>
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="text-sm text-[#5d5c64]">{{$sancion->tiposancion}}</div>
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="text-sm text-[#5d5c64] line-clamp-2 max-w-[400px]">{{$sancion->descripcion}}</div>
                                    </td>
                                            ...
                                            <div>
                                                ...
                                                @else
                                                    <button class="p-1.5 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all duration-300 shadow-sm hover:shadow-md transform hover:scale-[1.02]"
                                                        wire:click="$emit('deleteSancion', {{$sancion->id}})">
        ...
        @else
            <div class="bg-white rounded-xl shadow-md p-8 text-center">
                <div class="flex flex-col items-center justify-center gap-3">
                    ...
                    <p class="text-lg font-medium text-[#5d5c64]">No existen sanciones registradas</p>
                </div>
            </div>
        @endif
    </div>
